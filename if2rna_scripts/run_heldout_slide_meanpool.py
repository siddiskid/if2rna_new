#!/usr/bin/env python3
"""Run held-out-slide OOD evaluation with mean-pooling Ridge baseline.

For each held-out slide S:
1) Train on samples where slide_name != S
2) Test on samples where slide_name == S
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Held-out-slide OOD with mean-pooling Ridge")
    parser.add_argument("--ref_file", required=True)
    parser.add_argument("--feature_dir", default="data/if_features")
    parser.add_argument("--output_dir", default="results/phase3_ood/heldout_slide_meanpool")
    ...
    for heldout in slides:
        train_df = df[df["slide_name"] != heldout].reset_index(drop=True)
    ...
    summary_df.to_csv(out_dir / "heldout_slide_summary.csv", index=False)
    ...
    if len(summary_df) and (summary_df["status"] == "ok").any():
        ...
        ]).to_csv(out_dir / "heldout_slide_aggregate.csv", index=False)

    print(f"Saved: {out_dir / 'heldout_slide_summary.csv'}")
    if len(summary_df):
