...
#SBATCH --job-name=if2rna_p3_slide_vis
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --mem=64G
#SBATCH --output=logs/phase3_heldout_slide_vis_%j.out
#SBATCH --error=logs/phase3_heldout_slide_vis_%j.err
...
python if2rna_scripts/run_heldout_slide_neural.py \
  ...
  --output_dir results/phase3_ood/heldout_slide_vis \
  --model_type vis \
  --num_epochs 60 \
  --batch_size 16 \
  --seed 42 \
