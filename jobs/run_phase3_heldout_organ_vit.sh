...
#SBATCH --job-name=if2rna_ood_vit
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
#SBATCH --mem=64G
#SBATCH --output=logs/phase3_ood_vit_%j.out
#SBATCH --error=logs/phase3_ood_vit_%j.err
...
python if2rna_scripts/run_heldout_organ_neural.py \
  ...
  --output_dir results/phase3_ood/heldout_organ_vit \
  --model_type vit \
  --num_epochs 60 \
  --batch_size 16 \
  --lr 1e-3 \
