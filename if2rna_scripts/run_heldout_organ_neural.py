#!/usr/bin/env python3
"""Held-out-organ OOD evaluation for IF2RNA neural models (ViS/ViT/Attention-MIL).

For each held-out organ:
1) Train on all other organs
2) Validate on a patient-level split from the training organs
3) Evaluate on held-out organ
"""
...
class AttentionMIL(torch.nn.Module):
    def __init__(self, num_outputs, input_dim, hidden_dim=512, attn_dim=256, dropout=0.1, device='cuda'):
        ...
        self.attn_v = torch.nn.Linear(hidden_dim, attn_dim)
        self.attn_u = torch.nn.Linear(hidden_dim, attn_dim)
        self.attn_w = torch.nn.Linear(attn_dim, 1)
        self.decoder = torch.nn.Sequential(torch.nn.LayerNorm(hidden_dim), torch.nn.Linear(hidden_dim, num_outputs))
...
class IFRNADataset(Dataset):
    def __init__(self, df: pd.DataFrame, feature_dir: str, rna_cols):
...
def infer_feature_dim(df: pd.DataFrame, feature_dir: str):
        ...
        try:
            with h5py.File(h5_path, "r") as f:
                if "cluster_features" in f:
                    return int(f["cluster_features"].shape[1])
        except Exception:
            continue
    return None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Held-out-organ OOD for ViS/ViT/Attention-MIL")
    ...
    parser.add_argument("--heldout_organs", nargs="*", default=None)
    ...
    # Match training cleanup behavior.
    ...
    if args.heldout_organs:
        organs = [o for o in organs if o in args.heldout_organs]

    feature_dim = infer_feature_dim(df, args.feature_dir)
    if feature_dim is None:
        raise RuntimeError("Could not infer feature dimension from feature files")

    summary_rows = []
    all_results = {"genes": [g[4:] if g.startswith("rna_") else g for g in rna_cols]}

    for heldout in organs:
        print(f"\n===== Held-out organ: {heldout} =====")
        train_pool = df[df["organ_type"] != heldout].reset_index(drop=True)
        test_df = df[df["organ_type"] == heldout].reset_index(drop=True)

        if len(train_pool) == 0 or len(test_df) == 0:
            summary_rows.append({"heldout_organ": heldout, "status": "skipped_empty_split"})
            continue

        train_df, val_df = split_train_val_by_patient(train_pool, valid_size=args.valid_size, seed=args.seed)

        train_ds = IFRNADataset(train_df, args.feature_dir, rna_cols)
        val_ds = IFRNADataset(val_df, args.feature_dir, rna_cols)
        test_ds = IFRNADataset(test_df, args.feature_dir, rna_cols)
        ...
        organ_out = out_root / f"heldout_{heldout.lower()}"
        ...
        best_path = organ_out / "model_best.pt"
        if best_path.exists():
        ...
        corr_df.to_csv(organ_out / "gene_correlations.csv", index=False)
        ...
        all_results[f"heldout_{heldout.lower()}"] = {
            "preds": preds,
            "real": real,
            "wsi_file_name": wsis,
            "organ_type": projs,
    ...
    summary_df.to_csv(out_root / "heldout_organ_summary.csv", index=False)
    ...
    if (summary_df["status"] == "ok").any():
        ...
        pd.DataFrame([agg]).to_csv(out_root / "heldout_organ_aggregate.csv", index=False)

    with open(out_root / "heldout_organ_predictions.pkl", "wb") as f:
        pickle.dump(all_results, f, protocol=pickle.HIGHEST_PROTOCOL)

    print(f"Saved: {out_root / 'heldout_organ_summary.csv'}")
    if len(summary_df) > 0:
